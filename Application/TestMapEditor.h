...
#pragma region // ButtonHandle
	...
	public:
		...
		engine::event::Event<>& Click()
		{
			return m_widget->Click;
		}
	};
#pragma endregion
...
#pragma region // dialog
	...
	public:
		...
		void Close()
		{
			m_system.QueueForRemoval(m_widget);
		}
		...
		ButtonHandle CreateButton(const PositionF& pos, const SizeF& size, const std::string& text)
		{
			...
			label->SetSize(size);
			label->SetAlignment(Widget::VerticalAlignment::Center, Widget::HorizontalAlignment::Center);
		...
		void RemoveButton(ButtonHandle buttonHandle)
		{
			m_system.QueueForRemoval(buttonHandle.m_widget);
			buttonHandle.m_widget->Hide();
		}
		...
		void OnEnter() override
		{
			...
			if (false)
			{
				...
				// create content for the viewport. this will be a TextList with many items and a min/max number of items to display
				{
					...
					viewport->Resized += [&](const SizeF& size)
						{
