...
namespace engine
{
	namespace math
	{
		...
		struct Rect
		{
			...
			const engine::math::Size<T> GetSize() const
			{
				return math::Size<T>{ GetWidth(), GetHeight() };
			}
		...
		using RectF = engine::math::Rect<float>;
...
namespace std
{
	...
	template<typename T>
	struct hash<engine::math::Rect<T>>
	{
		size_t operator()(const engine::math::Rect<T>& r) const noexcept
		{
