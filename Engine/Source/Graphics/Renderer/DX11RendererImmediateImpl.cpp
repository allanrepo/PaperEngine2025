...
void engine::graphics::dx11::renderer::DX11RendererImmediateImpl::SetClipRegion(const engine::math::RectF& region)
{
	m_clipRegion = region;
}
...
engine::math::RectF engine::graphics::dx11::renderer::DX11RendererImmediateImpl::GetClipRegion() const
{
	return m_clipRegion;
}
...
#pragma region // update vertex translate transform. this will be the position. 
	...
	m_UpdateConstantBuffer.vertex.translate.x = (-m_D3DViewPort.Width + size.width) / 2 + pos.x - pivot.x;
	m_UpdateConstantBuffer.vertex.translate.y = (m_D3DViewPort.Height - size.height) / 2 + -pos.y + pivot.y;
#pragma endregion
